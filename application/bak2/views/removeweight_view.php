    ...
    <?php echo form_open('removeweight' .'/'.$animal[0]['chart_num']);?>
      ...
      <div class="form-group">
        ...
        <select name="id" id="id" class="form-control">
          <option value="">Select a weight record</option>
          <?php
            foreach($weights as $weight) { ?>
              <option  value="<?= $weight['id'] ?>"><?= $weight['date'] ?> - <?=$weight['weight']?></option>
          <?php
            } ?>
        </select> 
</div>

     <input class="btn btn-success" type="submit" value="Remove Weight"/>
   </form>
