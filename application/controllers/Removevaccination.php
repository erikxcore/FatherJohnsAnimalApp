...
class RemoveVaccination extends CI_Controller {
 ...
 function index($chart_num)
 {
   ...
   {
     $this->remove_vaccination($this->input->post('id'),$chart_num);
 ...
 function remove_vaccination($id,$chart_num){
    $name = $this->vaccination->getVaccinationById($id)[0]['name'];
    $result = $this->vaccination->removeVaccination($id);

    if($result){
          $entry = "Vaccination " . $name . " for " . $chart_num . ' has been updated on ' . date('Y-m-d') . '. Vaccination entry has been removed.';
