...
class DisplayAnimal extends CI_Controller {
 ...
 if(!empty($animal[0]['adopter'])){
   ...
   $pdf =  $this->pdf->load();
    ...
    $pdfUploadPath = "uploads/Adoption_Contract_" .  $chart_num[0] . ".pdf";
    $pdfFilePath = "files/Adoption_Contract_" .  $chart_num[0] . ".pdf";
...
function getcompleteinfo($chart_num){
  ...
  if($cat){
    ...
    $pdfUploadPath = "uploads/Adoption_Contract_Info_" .  $chart_num[0] . ".pdf";
    $pdfFilePath = "files/Adoption_Contract_Info_" .  $chart_num[0] . ".pdf";
                ...
                foreach($vaccinations as $vaccination){
                  ...
                  foreach($vaccination_names as $vac_info){
                    if($vaccination['name'] == $vac_info['name']){
                        if(isset($vac_info['serial_num']) && $vac_info['serial_num'] != null && $vac_info['serial_num'] != ""){
                        $html .= " - Serial Number - " . $vac_info['serial_num'];
                        }
                        if(isset($vac_info['brand_name']) && $vac_info['brand_name'] != null && $vac_info['brand_name'] != ""){
                        $html .= " - Brand Name - " . $vac_info['brand_name'];
                        }
                        if(isset($vac_info['type']) && $vac_info['type'] != null && $vac_info['type'] != ""){
                        $html .= " - Type - " . $vac_info['type'];
                        }
                        if(isset($vac_info['expiration_date']) && $vac_info['expiration_date'] != null && $vac_info['expiration_date'] != ""){
...
function getmedicalinfo($chart_num){
   ...
   else
   {
    ...
    $pdfUploadPath = "uploads/Medical_Info_" . $chart_num[0] .".pdf";
    $pdfFilePath = "files/Medical_Info_" .  $chart_num[0] .".pdf";
    ...
    for ($i=1; $i<=($pagecount); $i++) {
        ...
        $pdf->UseTemplate($import_page);
              ...
              <tr>
                <td>Date Started</td>
                <td>Date Completed (If Applicable)</td>
...
function getnotes($chart_num){
  ...
  if($cat){
    ...
    $pdfUploadPath = "uploads/Animal_Notes_Info_" .  $chart_num[0] . ".pdf";
    $pdfFilePath = "files/Animal_Notes_Info_" . $chart_num[0] . ".pdf";
