...
function add($name,$chart_num,$run_num,$species,$breed,$date_of_arrival,$acquired,$acquired_how,$microchip_num,$age,$sex,$feeding_instructions,$status,$status_date,$behavior_strategy,$notes,$safer_complete,$picture,$user,$user_date){
$data = array(
'name' => $name,
'chart_num' => $chart_num,
'run_num' => $run_num,
...
function edit($name,$chart_num,$run_num,$species,$breed,$date_of_arrival,$acquired,$acquired_how,$microchip_num,$age,$sex,$feeding_instructions,$status,$status_date,$behavior_strategy,$notes,$safer_complete,$picture,$user,$user_date){
...
//no longer a requirement, not used in official capacity
  function getMedicationRequiredAnimals(){
   ...
   $query =    $this->db->query('SELECT animals.id,animals.name,animals.chart_num,vaccination.name as vaccination_name,vaccination.date_completed FROM animals JOIN vaccination ON vaccination.chart_num = animals.chart_num WHERE vaccination.date_completed < CURRENT_DATE + INTERVAL 5 DAY AND vaccination.date_completed > CURRENT_DATE - INTERVAL 5 DAY ');
   //improve to return only the unique row instead of repeat rows if an animal has multiple medications due
   return $query->result_array();
 }
            ...
            if (array_key_exists($k,$sql_parts['field'])){
              if ($sql_parts['field'][$k] != $v){
                ...
                switch($k){
                  ...
                  break;
                  case "picture":
                  if($sql_parts['field'][$k] != null && $sql_parts['field'][$k] != ""){
                    if($sql_parts['field'][$k] != $v){
                      $different[$k] = $k . " set to " . $sql_parts['field'][$k] . '</br> Previously, this value was : ' . $v . '</br>';
                    }
                  }
