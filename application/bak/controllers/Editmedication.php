...
class EditMedication extends CI_Controller {
 ...
 function index($chart_num,$id)
 {
  ...
  if($this->session->userdata('superuser') != 1){
   ...
   {
     $this->edit_medication($this->input->post('id'),$this->input->post('med_name'),$this->input->post('date_given'),$this->input->post('date_due'));
 ...
 function edit_medication($id,$name,$date_given,$date_due){
    $date_converted1 = date('Y-m-d', strtotime($date_given));
    $date_converted2 = date('Y-m-d', strtotime($date_due));

    $result = $this->medication->editMedication($id,$date_converted1,$date_converted2,$name);
