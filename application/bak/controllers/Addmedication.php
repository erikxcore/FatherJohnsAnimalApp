...
class AddMedication extends CI_Controller {
 ...
 function index($chart_num)
 {
   ...
   {
     $this->add_medication($this->input->post('chart_num'),$this->input->post('med_name'),$this->input->post('date_given'),$this->input->post('date_due'));
 ...
 function add_medication($chart_num,$name,$date_given,$date_due){
    $date_converted1 = date('Y-m-d', strtotime($date_given));
    $date_converted2 = date('Y-m-d', strtotime($date_due));
    $result = $this->medication->addMedication($chart_num,$date_converted1,$date_converted2,$name);
